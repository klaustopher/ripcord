...
describe Ripcord::JsonRPC::Response do
  let(:succeeded_response) do
    Ripcord::JsonRPC::Response.new({ person_id: 47_110_815 }, nil, "1742e8634a96e9d24a1891927803127c")
  end
  let(:erroring_response) do
    Ripcord::JsonRPC::Response.new(nil, { code: -32_700, message: "Could not parse JSON" },
  ...
  context ".valid_data?" do
    ...
    it "returns false when data has error and result" do
      expect(Ripcord::JsonRPC::Response.valid_data?({ jsonrpc: "2.0", id: "1",
                                                      error: { code: 4711, message: "some message" }, result: 50 })).to be_falsey
    end
    ...
    it "returns false when the error data has no code" do
      expect(Ripcord::JsonRPC::Response.valid_data?({ jsonrpc: "2.0", id: "1",
                                                      error: { message: "some message" } })).to be_falsey
    end
    ...
    it "returns false when the error message is not a string" do
      expect(Ripcord::JsonRPC::Response.valid_data?({ jsonrpc: "2.0", id: "1",
                                                      error: { code: 4711, message: { foo: 123, bar: 456 } } })).to be_falsey
    end

    it "returns true for a valid response with error" do
      expect(Ripcord::JsonRPC::Response.valid_data?({ jsonrpc: "2.0", id: "1",
                                                      error: { code: 4711, message: "some message" } })).to be_truthy
    end
